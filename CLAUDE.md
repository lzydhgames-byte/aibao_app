...
**Plan 1 + 2 + 3 + 4 + 5 + 6 + 6b + 7 + 8 + 9-A 全部实现并通过冒烟。**
Plan 7 完成（2026-05-14）：音频混音管线全部代码就位；MVP 暂不收 BGM 文件，纯 TTS 路径运行 + 降级链路实测有效。
Plan 8 完成（2026-05-15）：连续剧 + HEARTBEAT 伪推送（轻量版）落地。ep2 首次实现 LLM 明确承认上一集（"昨天和小熊玩得可开心"），剧情连续感工程化。
Plan 9-A 完成（2026-05-16）：Flutter MVP 客户端 4 屏跑通，真 OPPO 手机端到端听到爱宝讲故事。第一次出现"产品形态"。
当前下一步：Plan 9b（补 BOOTSTRAP / HEARTBEAT / storyline 续集 / 故事历史 UI） / Plan 10（部署上线），按用户选。
...
- **续集 PostCheck 硬约束**（Plan 8）：`RequireContinuity + PreviousElements`——LLM 输出必须命中至少 1 个上一集元素，否则 fallback
...
### Flutter 客户端（Plan 9-A 已实现）
- **4 屏**：login（手机号 + 6 位验证码 + 60s 倒计时）/ home（欢迎语 + 孩子卡片 + 生成 CTA）/ generate（一句话需求 + 5/10/15 分钟 + 5 风格 + waiting overlay）/ player（故事文本 + just_audio + audio_url 8s 轮询 + dispose）
- **技术栈**：Flutter SDK 3.29.3 + Riverpod 2.6.1 + dio + go_router + just_audio + flutter_secure_storage
- **平台**：Android（debug 模式真机 OPPO PJJ110 端到端通过；release / iOS / desktop 留给 Plan 9b/10）
