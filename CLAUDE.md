...
Plan 9c 进行中（2026-05-17 开战）：故事质量第一战。已修：时长系数 120 → 320 字/分钟、prompt 硬约束 ±10%、字数长度保护（最多 2 次反馈式重写）、挡位调整 5/10/15 → 3/5/8 分钟、SceneSeed 80 个种子破除同质化、续集连续性 hard-fail → warn-only、超时栈整体上调。**未修硬问题**：8min 实测仍频繁掉 fallback（红线词单字「砍」误伤导致 80 字罐头模板）；3min/5min fallback 模板长度严重不匹配。
当前下一步：明天继续 Plan 9c——红线词单字清理 + fallback 模板补长。
