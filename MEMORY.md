...
- **2026-05-18** — Plan 9c 第二战：内容过滤分级宽容化。系统清理 9 个误伤红线词（血/砍/杀/鬼/性/惩罚/天堂/诡异/吓人），把 horror + negative_values 整类降级为 warn-only，violence/sexual/political/dangerous_imitation 保留 hard-fail。修复"同 prompt 产同故事"——user role 末尾追加 SceneSeed + 多样性 mandate + 16-hex nonce 三件套破 Doubao prompt 缓存。长度保护阈值 70%→85%。15 个故事抽样实测：fallback 率 7%（从昨天 ~30%），SceneSeed 15/15 全唯一，平均 ~62s/故事。意外发现 PreCheck 拦反义教育 prompt（"不要嘲笑别人" → 400），明天专攻。AVD 替代 MuMu（MuMu profile 飘+adb reverse 自掉）。
