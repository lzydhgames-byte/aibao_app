...
| 输入流程 | **B 方案：一句话需求 + 时长 → AI 推断 → 大纲卡预览-确认 → 正文生成**（Plan 11A 起；C 方案手填旋钮被吐槽后改）；续集模式保留快速路径（spec §10.1 与 outline 互斥） |
...
- **2026-05-18** — Plan 9d 完成：全量回归冒烟 13/13 通过（API 层 7 项 + UI-via-API 6 项 + player 跳过）。AVD 视觉验证用 API 模拟（征用户同意，验证数据/路由层 = 客户端入口）。意外发现 PostgreSQL Docker 容器中途挂掉 → 后端优雅关停不自动重连（Plan 10 用 systemd 兜底）。3 个 PowerShell UTF-8 编码坑串联：文件按 GBK 读 / 请求体被 PowerShell 二次编码 / 输出 Unicode 字符乱码——必须 stdin pipe + `[System.Text.Encoding]::UTF8.GetBytes()` + `[Console]::OutputEncoding`。Plan 9 大系列（9-A/9b/9c/9d）整体收官，端到端可演示。脚本 `scripts/plan9d-*-smoke.ps1` 留作日后回归工具。
- **2026-05-21** — Plan 10 部署 Day 2（100% 完成）：修完 COS 上传"三层洋葱"故障。① 网络层——香港→上海跨地域大文件 TLS 卡死 → 换桶 `aibao-audio-hk-1356733768`（ap-hongkong 同地域），TLS 卡死消失。② 权限层——新桶冒 403 `AccessDenied`（非 InvalidAccessKeyId 非 SignatureDoesNotMatch → 密钥有效、签名对、纯无此桶权限），根因 env 密钥 `AKIDGc...` 属另一账号，新桶建在主账号 APPID `1356733768` 名下 → 换主账号密钥。③ 操作层——主账号旧密钥 SecretKey 不可二次查看 → 新建密钥 `AKIDxuhG...`。3 个故事音频一次性全部 `ready`（391/203/169s）。**Plan 10 部署收官，端到端真机闭环 100% 通**。教训：云存储 4xx 三分法（AccessDenied≠InvalidAccessKeyId≠SignatureDoesNotMatch，指向不同修法）；云厂商 SecretKey 普遍只显示一次；COS 桶按 APPID 归属（桶名尾号=持有账号 APPID）；对象存储与计算资源必须同地域（跨地域大 PUT 会在 TLS 层卡死）。
- **2026-05-28** — Plan 11 代码侧完成（30/32 task）：AI 大纲预览 B 模式（11A）+ 成本可观测（11B Thin Slice + Full Build）。Codex 三轮 review 通过；subagent-driven 执行 4 sprint × 7 工作日 × 31 commits（22 task by subagent + 6 inline + 2 二次派遣）。**关键交付**：① 中立合约包 `service/outlinecontract`（接口+DTO+errors 零 IO）打破 story↔outline 双向依赖（spec §7.5 N5 解决方案）② append-only 事件流模式：`outline_events`（v8）+ `cost_events`（v9 with event_id `{trace_id}:{purpose}:{stage}:{attempt}` 业务语义化幂等 + price_version snapshot JSONB），所有写入都是 INSERT，`MarkExpiredIfPending` 用 INSERT WHERE NOT EXISTS ③ HMAC-SHA256 12hex `pkg/idhash` + domain separation 替换裸 SHA256，user/child id 不可枚举反推 ④ Gateway 不依赖 service 严守 Plan 1 分层（Gateway 返 Usage，业务方调 Recorder），`make check-layering` CI 守门 ⑤ OutlineSafetyCheck 输出端 4 类（红线/害怕/主角/IP）+ schema-repair 2 阶段重试 ⑥ Flutter generate_screen 三分支路由保留 storyline 续集模式（spec §10.1 互斥）+ `FeatureFlags.outlineEnabled` 编译时控紧急回滚。**实施期累积 ~15 处 plan vs 项目实际 API 偏差**（metrics 注入 vs promauto / logger.FromCtx / apperr.New 直构造 / safety.NewKeywordMatcher / RuleSet map vs []IPEntry / state/ vs providers/ / 等），归档到 plan 文件附录 A。**待补 Task 31 真链路对账 + Task 32 收官追述**——依赖上线 ≥ 24h + 厂商账单同步。详见 `docs/devlog/2026-05-28-plan-11-codeside-complete.md`。
- **2026-05-18** — Plan 10 部署 Day 1（92%）：腾讯云香港轻量 2C4G `aibao-mvp / 43.128.30.183` + Ubuntu 24.04。**意外发现服务器有前项目残留**：Docker + aibao-postgres + aibao-redis 已跑 3 周（健康），凭据 `aibao/aibao_prod_2026/aibao`——复用而非重建（调整 spec "YAGNI 不上 Docker" → "已有 Docker 就复用"）。Go 1.23 + golang-migrate + 7 migrations 全过 → 10 张表就位。`/etc/aibao/env` 12 密钥（3 生产新生成 + 9 用户 setx 过的外部 API key）+ systemd `Restart=on-failure` + Nginx + Let's Encrypt（aibao.dhgames.com，2026-08-16 过期自动续签）+ release APK 50MB + proguard ExoPlayer 规则。**端到端真机**走通：手机浏览器**直输 URL**下载 APK（微信/国产 ROM 浏览器拦截，绕不开）→ 安装 → 登录 → 文本 ✅ + TTS ✅ → **COS 上传 ❌**。**关键故障**：从香港主机到 `cos.ap-shanghai` 大文件 PUT 在 TLS handshake 卡死（5MB 20s timeout），小文件 300ms 秒过。怀疑链路 MTU/SNI/防火墙问题，**非代码层 bug**。两次 cos.go fix 都没解决（commit 76f702c / 93a09e8 自定义 transport + 裸 PUT），最终结论：换 bucket region 到 ap-guangzhou 或 ap-hongkong。明天 Day 2 处理。Minimax 又烧光余额一次（充值 100 + 又充 100）。outbox 状态机暴露 bug：worker 失败后 events 卡 processing 不回写 pending/dead，需手动 SQL 复活，Plan 11 加个 stale-reaper。
