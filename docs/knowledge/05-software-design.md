...
## 5.17 Composer/Handler 解耦：handler 持小接口，service 持完整结构

当 worker handler 需要调用一个有复杂依赖的 service（如 `audio.Orchestrator` 持有 TTS / BGMRepo / Cache / Mixer 4 个 dep），**不要让 handler 直接持有 `*audio.Orchestrator`**——会让 handler 单元测试必须 mock 4 个底层依赖。

**正确做法**：在 handler 包内定义一个**只声明 handler 需要的方法**的 tiny interface：
```go
// 在 worker/handlers 包里
type Composer interface {
    Compose(ctx context.Context, req audio.ComposeRequest) (*audio.ComposeResponse, error)
}
```

production：`audio.Orchestrator` 隐式满足这个接口（Go 鸭子类型，不需要 `implements` 声明）。
test：写个 `stubComposer{ wantResp *ComposeResponse, wantErr error }` 在 handler 包内，全测试通过 stub 完成。

**为什么需要**：这是 Interface Segregation Principle（ISP）的实操——客户端（handler）只依赖它实际使用的最小接口，不依赖 service 的"完整公开 API"。换实现 / 改 service 内部都不影响 handler 测试。**如果让 handler 持 `*audio.Orchestrator`**：
- handler 单测要 mock 4 个底层 dep（TTS、BGMRepo、Cache、Mixer），写一遍要 30 分钟
- service 改个签名 / 加个 dep → handler 测试全 break
- 接口"上提"到 service 端 → 接口臃肿、客户端要看一堆它用不上的方法

**项目体现**：
- Plan 7 Task 9 `worker/handlers/tts_synthesis.go` 定义 `Composer` 接口；handler 单测全程用 `stubComposer`，零 mock 底层 dep
- Plan 6 类似在 `bootstrap/service.go` 定义 `ChildLookup` 接口——bootstrap 只需要 `Get(ctx, id)`，不需要 child service 的完整 CRUD

**反模式**：让 service 自己定义所有可能客户端的接口（接口"上提"到 service 端），导致接口臃肿、改一处影响全部。**Go 习惯接口由消费者声明**，正反过来。这是 Java/C# 转 Go 最常踩的反向——Java 里 interface 通常和 class 放一起作为"我提供什么"的契约；Go 里 interface 是"我需要什么"的契约，由调用方写。
