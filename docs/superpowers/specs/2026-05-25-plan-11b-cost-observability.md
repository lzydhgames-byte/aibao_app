...
**与 outline_events 双表一致性**（呼应 11A §5.5 N1）：
- `outline_events` 同步、`cost_events` 异步 → 短暂窗口内可能"outline 已 accepted 但 cost 还在队列"
- 报表 JOIN 时**以 outline_events 为权威**；cost_events 缺行视为待补
- 定时一致性扫描：每小时 job 找"24h 内 outline accepted 但 cost_events 缺 outline LLM 行"的孤儿，写 `cost_outline_join_miss_total` metric；持续 >0.5% 触发告警
- **双表不共享事务**：钱已经花掉就是花了，cost_events 写失败不能反向回滚 outline 状态
...
`cost_events.outcome` 是**调用结果**（ok/fail/fallback），与 11A `outline_events.outcome` 的**生命周期**（pending/accepted/refreshed/expired）是两套不同枚举，**不直接对齐**：
...
CREATE TABLE cost_events (
    ...
    event_id        VARCHAR(96) NOT NULL UNIQUE,  -- 业务语义幂等键，见 §5.1.1
...
### 5.1.1 event_id 业务语义生成规则（N3）

**格式**：`{trace_id}:{purpose}:{stage}:{attempt}`
例：`ab12cd34ef56:outline:llm_call:1` / `ab12cd34ef56:story:llm_call:2` / `ab12cd34ef56:tts:synthesize:1`

**字段语义**：
- `trace_id` — 16 hex 字符，从 traceid middleware 拿（已有，Plan 1 实现）
- `purpose` — 同 cost_events.purpose 枚举（outline/story/tts/chapter_hook/memory_summary/storage_put）
- `stage` — 该 purpose 下的具体调用阶段（如 outline 的 `llm_call` / `safety_repair`；story 的 `llm_call` / `length_retry`）
- `attempt` — 同一 (trace_id, purpose, stage) 内的重试序号（从 1 起）

**为什么这样切**（N3 修正——之前 spec 写"trace_id + monotonic counter"在跨进程/重启不可靠）：
- 进程重启后 monotonic counter 归零 → 与重启前可能撞 ID
- 业务语义键不依赖进程内状态，重启/分布式都一致
- 同一 trace_id 下不同 stage 自然不冲突；同 stage 内 attempt 由业务方递增（如 LLM 重试时 retry_count++）
- ON CONFLICT(event_id) DO NOTHING 保证幂等：万一 Recorder 被重复调用（中间件/重试），第二次 INSERT 静默丢弃

**实施约束**：业务方传 event_id 字段；Recorder **不**自动生成（避免隐藏依赖）。Recorder 入参校验非空 + 格式正则，否则返 error。
...
**Hot-reload 立场（N2）**：本期**明确不支持** PriceBook 运行时热加载。理由：
- 改价是低频运维事件（季度级），不值得为此引入并发快照/原子切换/竞态防护代码
- 运行中改价存在"半数请求按旧价、半数按新价"的窗口，对账会模糊
- **正式流程**：改 `config.yaml` 单价 + bump `price_book_version` → `systemctl restart aibao` → 重启日志记录新版本号
- 运维 checklist 写入 13-deployment.md 知识库；新版本 deployment 必须包含 PriceBook 校对
...
**Secret rotation 策略（N4）**：

正常运行：**不轮换**。理由：轮换会使历史 hash 与新 hash 不可对齐，破坏跨月聚合。MVP 阶段单实例部署，泄漏面有限。

**泄漏应急流程**（手工事件，非自动化）：
1. **立即**生成新 secret（保留旧 secret 备查）
2. **不在线重算历史**——历史 `user_id_hash` / `child_id_hash` 保持旧值
3. 新数据用新 secret hash；spec 接受"分桶失真期"——本月活跃 Top200 排行表跨 rotation 时刻会断
4. 长期对账：若必须跨 rotation 对账，离线 migration 脚本拿 user_id 明文（PG 主库还在）重 hash 一次，覆盖历史 `user_id_hash` 字段（手工跑、不在线、不可逆）
5. 事件后审计：写 devlog 记录何时、为何 rotation、影响哪些 cost_events 行

**未来扩展**（不在本期实现）：双 secret 滚动期 + hash key version 字段，支持无损 rotation。需求驱动型，非现在。
...
样例输出（数字均为口径示例，非真实定价；以 PriceBook 校对 + cost_events 真实写入为准）：
```
Period: 2026-04-01 to 2026-04-30
PriceBook version: v20260525-1

=== Overall ===
Total spent:               ¥XXX.XX
Stories accepted:          XXX     (avg full pipeline ¥X.XX = LLM正文 + TTS + hook + memory + storage)
Outlines previewed:        XXX     (acceptance rate XX.X%)
  └─ accepted:             XXX
  └─ refreshed (作废):      XXX
  └─ expired:              XXX

=== Outline 省钱估算（口径见 11B §3.4）===
未进入正文的 outline 数:    N
  实际花费（仅大纲 LLM）:    ¥A.AA     <- SUM(cost_events for those outline_ids)
  若继续生成的预估花费:      ¥B.BB     <- N × avg(full_pipeline_cost_per_accepted_story)
  净省（B − A）:            ¥C.CC
...
=== Top 5 Users (HMAC-hashed) ===
user_hash      stories  outlines  total
3f8a2b9c1d04     42        58     ¥...
7c1d9e2f4a18     38        51     ¥...
...
```
...
| PriceBook config 改完忘记重启 | spec §5.2 明确不支持 hot-reload；改价必须重启（运维 checklist + 13-deployment.md 标注） |
